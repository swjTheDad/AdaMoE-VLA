...
class MoEType(Enum):
    """MoE implementation types."""

    VANILLA = "vanilla"  # Standard MoE with top-k routing
    ADAMOE = "adamoe"  # AdaMoE with democratic initialization
    CSMOE = "csmoe"  # Concatenated Scale Adapter MoE
...
class Config:
    ...
    head_dim: int
    use_moe: bool = True
    moe_type: MoEType = MoEType.ADAMOE
        ...
        for i, (x, config) in enumerate(zip(xs, self.configs, strict=True)):
            if x is not None:
                ...
                if i == self.action_expert_id:
                    if config.use_moe:
                        # Choose MoE implementation based on config
                        if config.moe_type == MoEType.VANILLA:
                            (
                                routed_expert_output,
                                moe_info
                            ) = moe.VanillaMoEFeedForward(
                        ...
                        elif config.moe_type == MoEType.CSMOE:
                            (
                                routed_expert_output,
                                moe_info
                            ) = moe.CSMoEFeedForward(
                                expert_dim=config.width,
                                hidden_dim=config.mlp_dim,
                                num_experts=self.num_experts,
                                top_k=self.top_k,
                        ...
                        else:  # MoEType.ADAMOE (default)
                            (
                                routed_expert_output,
                                moe_info
                            ) = moe.AdaMoEFeedForward(
                                expert_dim=config.width,
                                hidden_dim=config.mlp_dim,
                                num_experts=self.num_experts,
                                top_k=self.top_k,
                        ...
                        routed_expert_output = sharding.activation_sharding_constraint(
                            routed_expert_output
                        )
                        if self.use_general_expert:
                            general_expert_output = FeedForward(  # noqa: PLW2901
                                features=config.width,
                                hidden_dim=config.mlp_dim,
                                name=_name("mlp", i),
                            )(x)
                            general_expert_output = (
                        ...
                        # Fallback to original pi0 implementation (standard FeedForward)
...
class Module(nn.Module):
    ...
    def setup(self):
        ...
        self.layers = nn.scan(
            ...
            in_axes=(
                ...
                nn.broadcast,
                nn.broadcast,
            ),  # 0=kv_cache, 1=positions, 2=mask, 3=decode
    ...
    def __call__(
        ...
        *,
        ...
        KVCache,
        dict[str, at.Float[at.Array, ""]] | None
    ]:
        embedded = jax.tree.map(lambda e: e.astype(self.embed_dtype), embedded)
        mask = jnp.asarray(mask)[:, None, :, :]

        embedded, (kv_cache, moe_info) = (
            self.layers(embedded, kv_cache, positions, mask, deterministic)
        )
        ...
        if moe_info is not None:
            moe_info = {k: jnp.mean(v) for k, v in moe_info.items()}
        return (
            [
                f(e) if e is not None else e
                for f, e in zip(self.final_norms, embedded, strict=True)
            ],
            kv_cache,
            moe_info
        )
