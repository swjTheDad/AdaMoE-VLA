    ...
    *,
    ...
    def init(
        ...
        params = nnx_utils.state_map(
            ...
            # lambda p: p.replace(p.value.astype(jnp.float32))
        )
...
def train_step(
    ...
    def loss_fn(
        ...
        def warmup_branch(_):
            return (
                jnp.mean(chunked_loss) + jnp.mean(moe_info["load_balancing_loss"] * 0.01),
                # jnp.mean(chunked_loss) + jnp.mean(z_loss),
                (
                    moe_info,
                    jnp.mean(chunked_loss),
                ),
            )

        def normal_branch(_):
            return (
                jnp.mean(chunked_loss) + jnp.mean(moe_info["load_balancing_loss"] * 0.005),
                (
                    moe_info,
                    jnp.mean(chunked_loss),
                ),
    ...
    (loss, aux_info), grads = nnx.value_and_grad(
        loss_fn, argnums=diff_state, has_aux=True
    )(model, train_rng, observation, actions)
    moe_info, flow_matching_loss = aux_info
    ...
    moe_filter = nnx_utils.PathRegex(
        ".*moe.*",
    )
    router_filter = nnx_utils.PathRegex(
        ".*moe.*gating.*",
    )
    kernel_moe = nnx.All(
        nnx.Param,
...
def main(config: _config.TrainConfig):
    ...
    def get_param_info(params_tree):
        ...
        total_params = sum(
            v.size if hasattr(v, "size") else 0 for v in flat_params.values()
        )
    ...
    logging.info(
        f"Trainable parameters: {trainable_param_count:,} ({trainable_param_count/total_param_count*100:.1f}%)"
    )
    logging.info(
        f"Frozen parameters: {frozen_param_count:,} ({frozen_param_count/total_param_count*100:.1f}%)"
    )
    logging.info("")
    ...
    for step in pbar:
        ...
        if step % config.log_interval == 0:
            ...
            info_str = ", ".join(
                (
                    f"{k}={float(v):.4f}"
                    if isinstance(v, (float, jnp.floating))
                    else f"{k}={v}"
                )
        ...
        if (
            ...
            if step == config.num_train_steps - 1:
                _checkpoints.save_state(
                    checkpoint_manager, train_state, data_loader, step + 1
                )
            else:
                _checkpoints.save_state(
